...
let contract;
let contractAddress = "0x8adf52d2825Cc09d1be177D12c94A66a9e89641D"; // G2 beta 6
let provider;
let signer = 0;
let filter;
...
function checkHatch() {
	if(m_account !== a_king) {
		...
		notificationCondition("Cannot hatch while King.");
	}
}
...
function localShroomUpdate() {
	...
	document.getElementById('shroom').innerHTML = m_localShroom; 
}
...
function updatePullCost() {
    ...
    {
        a_pullCost = parseInt(result.toString()) + parseInt(1);
...
// King Glory (in progress)
function updateKingGlory() {
	contract.ComputeGlory().then((result) =>
	{
		m_kingGlory = result.toString();
		document.getElementById('kingGlory').innerHTML = m_kingGlory;
	})
}
...
function beginEventLogging() {
	...
	contract.on("WithdrewBalance", (sender, eth, event) => {
		let _string = formatEthAdr(sender) + " has withdrawn " + truncateEther(eth) + " POA to their wallet." ;
		checkEventPast(_string, event.blockNumber);
	});
	...
	contract.on("Pushed", (sender, eth, event) => {
		let _string = formatEthAdr(sender) + " claims the throne by spending " + truncateEther(eth) + " POA. 200 Eggoas join their nest.";
		checkEventPast(_string, event.blockNumber);
	});
	...
	contract.on("Overthrown", (sender, glory, event) => {
		let _string = formatEthAdr(sender) + " leaves the throne! Their reign awards them " + glory.toString() + " Glory.";
		checkEventPast(_string, event.blockNumber);
	});
